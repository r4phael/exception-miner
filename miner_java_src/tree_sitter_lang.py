...
QUERY_FUNCTION_DEF: Query = JAVA_LANGUAGE.query(
    f"({METHOD_DECLARATION}) @function.def")
...
QUERY_TRY_EXCEPT: Query = JAVA_LANGUAGE.query(
    f"""[
        (try_with_resources_statement) @try.stmt
        (try_statement) @try.stmt
    ]
    (catch_clause) @except.clause""")

QUERY_EXCEPT_TYPE: Query = JAVA_LANGUAGE.query(
    f"(catch_type) @catch.type")

QUERY_EXCEPT_CLAUSE: Query = JAVA_LANGUAGE.query(
    f"({CATCH_CLAUSE}) @except.clause")
...
QUERY_RAISE_STATEMENT: Query = JAVA_LANGUAGE.query(
    """(throw_statement) @raise.stmt""")
