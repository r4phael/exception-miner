...
def get_try_slices(node: Node):
    ...
    for capture in captures:
        if(capture[1] == 'try.stmt' and flag):
            break
        else:
            flag = True
...
def get_try_catch_slices(node: Node):
    ...
    while(len(queue) != 0):
        child = queue.pop()
        if(len(child.children) == 0 and child.type not in ['line_comment', 'block_comment']):
...
def count_try_except_raise(node: Node):
    ...
    for i in range(len(captures)):
        if(captures[i][1] == 'raise.throw'):
            continue
        if(captures[i][1] == 'raise.type.parameter' and captures[i][0].text == b'Exception' and captures[i+1][1] == 'raise.throw'):
            cnt += 1
...
def remove_emojis(string):
    emoji_pattern = re.compile(
        ...
        u"\U0000200B" # empty space unicode
        u"\U0000200C" # empty space unicode
        "`qި"
        "]+", 
        flags=re.UNICODE
    )
